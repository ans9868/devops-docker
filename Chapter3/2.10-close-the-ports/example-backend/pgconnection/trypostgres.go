...
func TryPostgres() (bool, error) {
	...
	if err != nil {
		fmt.Println(err)
		fmt.Println("[DEBUG] Ran query to check pong message")
...
func InitializePostgresClient() error {
	fmt.Println("[DEBUG] InitializePostgresClient() called")
	postgresHost := os.Getenv("POSTGRES_HOST")
	fmt.Println("[DEBUG] POSTGRES_HOST =", postgresHost )
	...
	fmt.Println("[DEBUG] pg.Connect() called, trying to ping...")
	...
	for i := 0; i <= 10; i++ {
		...
		if i < 10 {
			fmt.Println("[Ex 2.6+] Connection to postgres failed! Retrying...")
				fmt.Printf("[Ex 2.6+] Connection to postgres failed! Retrying... (attempt %d) — error: %v\n", i+1, err)
